...
@click.command(help="""Encrypts or decrypts files using AES-256.
...
""")
@click.option("--mode", required=True, type=click.Choice(["encrypt", "decrypt"]), help="Operation mode")
...
def cli(mode, password, ext, scan_path, out_dir):
    ...
    out_dir = Path(out_dir) if out_dir else Path("encrypted" if mode == "encrypt" else "decrypted")
    out_dir.mkdir(parents=True, exist_ok=True)

    files = [f for f in scan_path.rglob("*") if f.is_file() and (not ext or f.suffix == f".{ext}")]
