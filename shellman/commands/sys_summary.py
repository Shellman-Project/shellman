...
@click.command(
    help="""Shows system, shell, and tool environment summary.
...
def cli():
    ...
    try:
        ...
        up2 = subprocess.run(
            ["uptime"], stdout=subprocess.PIPE, text=True
        ).stdout.strip()
        print(f"Uptime       : {up1}")
    ...
    try:
        ...
        ip_public = (
            urllib.request.urlopen("https://ifconfig.me").read().decode().strip()
        )
    except Exception:
    ...
    try:
        if pkg_mgr == "apt":
            count = int(
                subprocess.run(["dpkg", "-l"], stdout=subprocess.PIPE)
                .stdout.decode()
                .count("\nii")
            )
        elif pkg_mgr == "pacman":
            count = int(
                subprocess.run(["pacman", "-Q"], stdout=subprocess.PIPE)
                .stdout.decode()
                .count("\n")
            )
        elif pkg_mgr == "dnf":
            count = int(
                subprocess.run(["dnf", "list", "installed"], stdout=subprocess.PIPE)
                .stdout.decode()
                .count("\n")
            )
        elif pkg_mgr == "brew":
            count = int(
                subprocess.run(["brew", "list"], stdout=subprocess.PIPE)
                .stdout.decode()
                .count("\n")
            )
        elif pkg_mgr == "choco":
            count = int(
                subprocess.run(["choco", "list", "-l"], stdout=subprocess.PIPE)
                .stdout.decode()
                .count("\n")
            )
        else:
