...
def test_basic_replace(tmp_path):
    ...
    result = runner.invoke(cli, [
        str(tmp_path), "--find", "hello", "--replace", "hi", "--ext", "txt"
    ])
...
def test_in_place_replace(tmp_path):
    ...
    runner = CliRunner()
    result = runner.invoke(cli, [
        str(tmp_path), "--find", "replace", "--replace", "fixed", "--ext", "md", "--in-place"
    ])
...
def test_confirm_skips_file(tmp_path):
    ...
    runner = CliRunner()
    result = runner.invoke(cli, [
        str(tmp_path), "--find", "maybe", "--replace", "definitely", "--ext", "txt", "--in-place", "--confirm"
    ], input="n\n")
...
def test_preview_diff_shown(tmp_path):
    ...
    runner = CliRunner()
    result = runner.invoke(cli, [
        str(tmp_path), "--find", "replace", "--replace", "fix", "--ext", "txt", "--preview"
    ])
